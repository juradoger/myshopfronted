          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {products.map((product) => {
                const { discountPercent, originalPrice } = calculateDiscount(product.price)
                return (
                  <div key={product.id} className="group">
                    ...
                    <div className="mt-1 flex items-center justify-between">
                      <p className="text-sm font-medium">Ahora ${product.price}</p>
                      <p className="text-sm text-gray-500 line-through">${originalPrice}</p>
                    </div>
                    <div className="mt-2 flex gap-1">
                      <button className="h-4 w-4 rounded-full bg-black"></button>
                      {product.color === "black" && <button className="h-4 w-4 rounded-full bg-black"></button>}
                      {product.color === "blue" && <button className="h-4 w-4 rounded-full bg-blue-600"></button>}
                      {product.color === "red" && <button className="h-4 w-4 rounded-full bg-red-600"></button>}
                      <button className="h-4 w-4 rounded-full bg-amber-700"></button>
                      <button className="h-4 w-4 rounded-full bg-gray-700"></button>
                    </div>
                    {product.category === "men's clothing" ? (
                      <p className="mt-1 text-xs text-gray-500">MATERIAL RECICLADO • HECHO EN ESPAÑA</p>
                    ) : (
                      <p className="mt-1 text-xs text-gray-500">+ COLORES VARIADOS</p>
                    )}
                  </div>
                )
              })}
